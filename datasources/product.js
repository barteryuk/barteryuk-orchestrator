...
class Controller {
  ...
  static async findOne(parent, args, context, info) {
    ...
    try {
      ...
      const [filtered] = arr.filter(
        (el) => String(el._id) == String(productid)
      );
  ...
  static async findOwnItems(parent, args, context, info) {
    ...
    try {
      ...
      if (products && products != "") {
        if (products.data) {
          ownItems = products.data.filter(
            (product) => product.userId == payload._id
          );
        } else {
          ownItems = products.filter(
            (product) => product.userId == payload._id
          );
        }
      } else {
  ...
  static async bidItem(parent, args, context, info) {
    ...
    try {
      ...
      console.log("WHAT'S BIDPRODUCTID");
      console.log(
        bidmsg.result.bidProductId[bidmsg.result.bidProductId.length - 1],
        "\n"
      );
